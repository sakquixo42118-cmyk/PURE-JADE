"""Run the full PURE-JADE v0.2.1 chain.

Chain:
    first_v021_boundary_copy -> run_strategy_pipeline_v021.py
    -> third/behavior_generator_api_schema_aligned.py
    -> forth/run_full_evaluation.py

This runner is an orchestration layer only. It does not modify the original
first or forth source files. Reports are grouped by conversation:
...
FIRST_V021_DIR = SCRIPTS_DIR / "first_v021_boundary_copy"
THIRD_DIR = SCRIPTS_DIR / "third"

DEFAULT_ENV_FILE = PROJECT_ROOT / ".env"
DEFAULT_REFERENCES = PROJECT_ROOT / "examples" / "strategy-references-v0.1.json"
DEFAULT_DIMENSIONS = PROJECT_ROOT / "scripts" / "forth" / "evaluation_dimensions.json"
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "reports" / "full_chain_v021"
...
def default_conversation_id() -> str:
    return "chain_v021_" + time.strftime("%Y%m%d_%H%M%S")
...
def run_first_stage(args: argparse.Namespace, output_dir: Path) -> tuple[Path, dict[str, Any]]:
    first_main = load_module(
        "pure_jade_first_v021_main",
        FIRST_V021_DIR / "main.py",
        prepend_paths=[FIRST_V021_DIR],
    )
    ...
    first_report = {
        "status": "pass",
        "stage": "first_v021_boundary_copy",
...
def prepare_working_record(source_record: Path, output_dir: Path) -> Path:
    working_record = output_dir / "conversation_record_v021_chain.json"
...
def run_strategy_stage(args: argparse.Namespace, working_record: Path, output_dir: Path, turn_id: int) -> dict[str, Any]:
    strategy_module = load_module(
        "pure_jade_strategy_v021",
        SCRIPTS_DIR / "run_strategy_pipeline_v021.py",
        prepend_paths=[SCRIPTS_DIR],
    )
    api_client = load_module(
        "pure_jade_api_for_chain",
        SCRIPTS_DIR / "pure_jade_api.py",
        prepend_paths=[SCRIPTS_DIR],
    )
...
def run_behavior_stage(args: argparse.Namespace, working_record: Path, output_dir: Path, turn_id: int) -> dict[str, Any]:
    behavior_module = load_module(
        "pure_jade_behavior_schema_aligned",
        THIRD_DIR / "behavior_generator_api_schema_aligned.py",
        prepend_paths=[THIRD_DIR, SCRIPTS_DIR],
    )
...
def build_evaluation_cases(working_record: Path, output_dir: Path, turn_id: int) -> Path:
    ...
    write_json(
        ...
        {
            "schema_version": "0.2.1",
            "purpose": "full_chain_v021_current_turn_evaluation_input",
...
def build_all_turn_evaluation_cases(working_record: Path, output_dir: Path) -> Path:
    ...
    write_json(
        ...
        {
            "schema_version": "0.2.1",
            "purpose": "full_chain_v021_all_turns_evaluation_input",
            "source_record": str(working_record),
...
def run_evaluation_stage(
    ...
    command = [
        sys.executable,
        str(SCRIPTS_DIR / "forth" / "run_full_evaluation.py"),
        "--cases",
        str(cases_path),
        "--dimensions",
        str(args.dimensions),
...
def write_conversation_summary_report(
    ...
    summary = {
        "report_type": "pure_jade_v021_conversation_summary",
        "schema_version": "0.2.1",
        "conversation_id": record.get("conversation_id"),
        "current_turn_id": record.get("current_turn_id"),
...
def write_dialogue_review_report(
    ...
    review = {
        "report_type": "pure_jade_v021_dialogue_review",
        "schema_version": "0.2.1",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Pass-through dimensions file for scripts/forth/run_full_evaluation.py.",
    )
    parser.add_argument(
        "--eval-stage",
