...
ALLOWED_PRACTICAL_URGENCIES = ["none", "low", "medium", "high"]
ALLOWED_CONSEQUENCE_DOMAINS = [
    "none",
    "exam",
    "deadline",
    "administrative",
...
ALLOWED_ACTIONABILITY = [
    "not_actionable",
    "unclear",
    "later_action_possible",
    "immediate_action_possible",
]
REAL_WORLD_EVENT_TERMS = (
    "考试",
    "期末",
    "补考",
    "缓考",
...
3. 输出必须包含以下字段：
   ...
   new_evidence, revised_fields, open_questions, risk_memory,
   practical_urgency, real_world_consequence, consequence_domain, actionability, confidence。
...
8. need 只能从以下标签选择：被理解、被肯定、情绪陪伴、信息澄清、解决方案、现实补救、事实资源、安全支持、表达空间、其他。
...
16. practical_urgency 只能是 none, low, medium, high；它表示现实任务/后果是否紧急，不等同于自伤风险。
17. real_world_consequence 必须是 boolean；当用户遇到考试、截止、报名缴费、面试、预约、交通等现实后果事件时输出 true。
18. consequence_domain 只能是 none, exam, deadline, administrative, interview, appointment, transport, medical, legal, finance, relationship, other。
19. actionability 只能是 not_actionable, unclear, later_action_possible, immediate_action_possible。
...
现实任务规则：
- risk_level 只表示安全风险；低风险不等于事情不紧急。
- 如果用户说错过考试、DDL/截止已过、报名缴费/预约/面试/交通行程出问题，应保留情绪需求，同时把 real_world_consequence 设为 true。
- 上述场景通常 practical_urgency 为 medium 或 high，actionability 为 immediate_action_possible，need 至少包含 情绪陪伴 和 现实补救/信息澄清。
- 不要把这类场景压缩成单纯“情绪陪伴”；support_stage 可以是 action，即使用户情绪很强。
"""
...
- 不进行医学或心理诊断，不提供医疗建议。
- 先确认和理解用户的情绪，再回应内容。
- 普通低风险回复通常保持 3-6 句，温和、自然、不评判；不要为了简短而只输出一句。
...
def _allowed_value(value, allowed: list[str], fallback: str) -> str:
...
def _evidence_text(state_card: dict, current_user_message: str) -> str:
    pieces = [current_user_message]
    for field in ("problem_summary", "dialogue_summary"):
        value = state_card.get(field)
        if isinstance(value, str):
            pieces.append(value)
    pieces.extend(item for item in _as_list(state_card.get("evidence")) if isinstance(item, str))
    pieces.extend(item for item in _as_list(state_card.get("new_evidence")) if isinstance(item, str))
    return " ".join(item for item in pieces if item)


def _infer_consequence_domain(text: str) -> str:
    lower_text = text.lower()
    ...
    if any(term in text for term in ("航班", "飞机", "火车", "高铁", "车票")):
        return "transport"
    if any(term in text for term in ("医院", "体检", "药", "病历")):
        return "medical"
    if any(term in text for term in ("报警", "法院", "合同", "罚款", "签证")):
        return "legal"
    if any(term in text for term in ("钱", "欠费", "付款", "转账")):
        return "finance"
    if any(term in text for term in ("分手", "吵架", "朋友", "父母", "家里")):
        return "relationship"
    return "other"


def _infer_practical_context(text: str) -> dict:
    has_event = _contains_any(text, REAL_WORLD_EVENT_TERMS)
    has_problem = _contains_any(text, REAL_WORLD_PROBLEM_TERMS) or bool(
        re.search(r"(错过|没赶上|忘了|来不及).{0,12}(考试|期末|面试|预约|报名|截止|ddl|deadline)", text.lower())
    )
    if not (has_event and has_problem):
        return {
    ...
    return {
        ...
        "consequence_domain": _infer_consequence_domain(text),
...
def _normalize_practical_context(state_card: dict, current_user_message: str) -> None:
    """Keep concrete real-world urgency from being lost in emotional summaries."""
    inferred = _infer_practical_context(_evidence_text(state_card, current_user_message))
    declared_real_world = state_card.get("real_world_consequence") is True
    real_world_consequence = inferred["real_world_consequence"] or declared_real_world

    if real_world_consequence:
        state_card["real_world_consequence"] = True
        state_card["practical_urgency"] = inferred["practical_urgency"] if inferred["real_world_consequence"] else _allowed_value(
            state_card.get("practical_urgency"), ALLOWED_PRACTICAL_URGENCIES, "medium"
        )
        state_card["consequence_domain"] = inferred["consequence_domain"] if inferred["real_world_consequence"] else _allowed_value(
            state_card.get("consequence_domain"), ALLOWED_CONSEQUENCE_DOMAINS, "other"
        )
        state_card["actionability"] = inferred["actionability"] if inferred["real_world_consequence"] else _allowed_value(
            state_card.get("actionability"), ALLOWED_ACTIONABILITY, "unclear"
        )
        state_card["need"] = _append_unique(state_card["need"], "情绪陪伴", "现实补救", "信息澄清")
        if state_card.get("risk_level") != "high":
            state_card["support_stage"] = "action"

        if state_card["consequence_domain"] == "exam":
            exam_question = "需要确认学校或课程的补考、缓考、教务沟通流程。"
            if not any("补考" in item or "缓考" in item or "教务" in item for item in state_card["open_questions"]):
                state_card["open_questions"].append(exam_question)
        return

    state_card["real_world_consequence"] = False
    state_card["practical_urgency"] = "none"
    state_card["consequence_domain"] = "none"
    state_card["actionability"] = "not_actionable"
...
def _normalize_state_card(
    ...
    try:
        ...
        intensity = 1
    _normalize_practical_context(state_card, current_user_message)
    if risk_level == "high":
