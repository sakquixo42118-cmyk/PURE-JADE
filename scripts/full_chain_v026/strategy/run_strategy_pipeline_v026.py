...
REQUEST_SCHEMA_VERSION = "0.2.6"
CARD_SCHEMA_VERSION = "0.2"
PIPELINE_SCOPE = "research_content_2_strategy_decision_multiturn_v026"
PIPELINE_VERSION = "strategy_pipeline_v0.2.6"
...
URGENT_PRACTICAL_URGENCIES = {"medium", "high"}
NON_BLOCKING_ACTIONABILITY = {"unclear", "later_action_possible", "immediate_action_possible"}
...
def consequence_domain_from_text(text: str) -> str:
    lower_text = text.lower()
    if any(term in text for term in ("考试", "期末", "补考", "缓考", "挂科")):
        return "exam"
    if any(term in lower_text for term in ("ddl", "deadline")) or any(term in text for term in ("截止", "提交", "作业", "答辩")):
        return "deadline"
    if any(term in text for term in ("报名", "缴费", "教务", "申请", "材料", "手续")):
        return "administrative"
    if "面试" in text:
        return "interview"
    if any(term in text for term in ("预约", "挂号")):
        return "appointment"
    if any(term in text for term in ("航班", "飞机", "火车", "高铁", "车票")):
        return "transport"
    return "other"


def infer_practical_context(message: str, user_state: dict[str, Any]) -> dict[str, Any]:
    state_need = [item for item in base.as_list(user_state.get("need")) if isinstance(item, str)]
    state_real_world = user_state.get("real_world_consequence") is True
    state_urgency = user_state.get("practical_urgency")
    state_actionability = user_state.get("actionability")
    state_domain = user_state.get("consequence_domain")

    text = " ".join(
        item
        for item in [
            message,
            user_state.get("problem_summary") if isinstance(user_state.get("problem_summary"), str) else "",
            user_state.get("dialogue_summary") if isinstance(user_state.get("dialogue_summary"), str) else "",
        ]
        if item
    )
    has_event = contains_term(text, URGENT_EVENT_ACTION_TERMS)
    has_problem = contains_term(text, URGENT_EVENT_PROBLEM_TERMS) or bool(
        re.search(r"(错过|没赶上|忘了|来不及).{0,12}(考试|期末|面试|预约|报名|截止|ddl|deadline)", text.lower())
    )
    inferred_real_world = has_event and has_problem
    real_world_consequence = state_real_world or inferred_real_world or "现实补救" in state_need

    if not real_world_consequence:
        return {
            "real_world_consequence": False,
            "practical_urgency": "none",
            "consequence_domain": "none",
            "actionability": "not_actionable",
            "support_need": state_need,
            "source": "none",
        }

    return {
        "real_world_consequence": True,
        "practical_urgency": state_urgency if state_urgency in {"low", "medium", "high"} else "high",
        "consequence_domain": state_domain if isinstance(state_domain, str) and state_domain != "none" else consequence_domain_from_text(text),
        "actionability": state_actionability
        if state_actionability in NON_BLOCKING_ACTIONABILITY
        else "immediate_action_possible",
        "support_need": state_need,
        "source": "state_card" if state_real_world else "local_inference",
    }
...
def build_strategy_decision_request(
    ...
    if not isinstance(user_state_card, dict):
        user_state_card = {}
    message = current_user_message(record, turn_id)
    ...
    return {
        ...
        "practical_context": infer_practical_context(message, user_state_card),
...
def reason_from_user_state(
    ...
    if isinstance(practical_context, dict) and practical_context.get("real_world_consequence") is True:
        parts.append(
            "现实任务上下文："
            f"domain={practical_context.get('consequence_domain')!r}, "
            f"urgency={practical_context.get('practical_urgency')!r}, "
            f"actionability={practical_context.get('actionability')!r}"
        )
...
def practical_context_is_urgent(strategy_request: dict[str, Any]) -> bool:
    context = strategy_request.get("practical_context")
    if not isinstance(context, dict):
        return False
    return (
        context.get("real_world_consequence") is True
        and context.get("practical_urgency") in URGENT_PRACTICAL_URGENCIES
        and context.get("actionability") in NON_BLOCKING_ACTIONABILITY
    )


def practical_decision() -> dict[str, Any]:
    return {
...
def practical_constraints(strategy_request: dict[str, Any]) -> list[str]:
    context = strategy_request.get("practical_context")
    ...
    if domain == "exam":
        constraints.append("现实下一步优先指向联系任课老师、教务或辅导员，询问补考/缓考/缺考处理流程；可同时建议准备简短说明或查看官方通知")
    elif domain in {"deadline", "administrative"}:
        constraints.append("现实下一步优先指向查看官方截止要求或联系负责人员确认补救流程；可同时建议先整理必要信息")
    else:
...
def quality_constraints_for_decision(decision: dict[str, Any]) -> list[str]:
    constraints = []
    for item in base.strategy_constraints(decision):
        if item == "先复述处境，不替用户下结论":
            constraints.append("不要机械复读用户原话；用 1-2 句把用户处境中的情绪张力具体化")
        elif item == "先承接情绪，不急于给建议":
            constraints.append("先承接情绪，并基于用户原话展开其心理负担、自责或关系压力")
        else:
    ...
    if support_intention in {"comfort", "affirm", "normalize"}:
        constraints.append("允许 evidence-grounded expansion：基于用户已说出的事实展开情绪处境、心理负担和轻度重构")
        constraints.append("回复应包含具体情绪承接、轻度重构和一个微行动或温和探索入口；微行动不是沉重建议")
    ...
    constraints.append("constraints 只写表达功能，不写可被下游照抄的成品句或固定示例句")
...
def rule_strategy_decision_card(
    ...
    if decision["support_intention"] != "safety_support" and practical_context_is_urgent(strategy_request):
    ...
    return {
        ...
        "response_goal": response_goal_v026(decision, strategy_request),
        "reason": reason_from_user_state(
            user_state,
            selected_ids,
            strategy_request.get("practical_context") if isinstance(strategy_request.get("practical_context"), dict) else None,
        ),
...
def response_goal_v026(decision: dict[str, Any], strategy_request: dict[str, Any]) -> str:
    if practical_context_is_urgent(strategy_request):
        return "先基于用户已说出的事实具体化其压力，再用 3-5 句提供不承诺结果的现实补救下一步，并以温和语气收束。"
    support_intention = decision.get("support_intention")
    if support_intention in {"comfort", "affirm", "normalize"}:
        return "在不添加新事实的前提下，具体展开用户处境里的心理张力和自责机制，再给一个很小的稳定动作或温和探索入口。"
    if decision.get("response_timing") == "offer_next_step":
        return "先基于用户原话承接核心困惑、不甘或自我怀疑，再给 1-2 个低负担、可执行的下一步，并避免编造或过度推测。"
...
def strategy_system_prompt_v026() -> str:
    return """你是 PURE-JADE 项目的共情策略决策模块。
你的任务是根据当前用户输入、最新用户状态卡和可选 ESConv 策略参考案例摘要，输出一张共情策略决策卡。

你只负责第二部分策略决策，不生成最终回复，不重新总结完整历史。v0.2.6 的输入会额外提供 practical_context，用来保留现实后果、紧急度和可行动性，不要忽略它。
必须遵守以下规则：
...
10. reason 必须基于当前用户输入、最新用户状态卡和 practical_context，不要补充没有依据的事实。
...
5. comfort / affirm / normalize 不应只停在“你不差、这很正常”。低风险场景可以允许一个很小的稳定动作、温和探索入口或自我评价缓冲，例如让用户先把最刺痛的比较写成一句话、暂停自我审判、或具体化一个场景。
6. action 阶段不仅限于用户显式说“怎么办”。当用户处在现实压力、冲突、拖延、学习失控、社交退缩等场景且已有足够上下文时，可以主动给少量可执行下一步。
7. 用户追问“为什么/问题在哪/我不理解”时，不要急着给大段原因推测。更稳的策略是先承接困惑和不甘，再帮助用户具体化事件、检查评价标准或请求反馈。
8. safety_support 时，response_timing 必须为 safety_override，response_intensity 必须为 directive；constraints 应允许一个必要的安全确认问题，但禁止追问危险方法、工具、具体计划或刺激性细节。
...
12. v0.26 的核心目标是 evidence-grounded expansion：克制不是短，克制是不编造。允许基于用户已经说出的事实展开心理张力、情绪处境和轻度重构；禁止添加未经证实的新事实、心理诊断、动机判断或责任归因。
13. 不要机械复读用户原话；但需要用 1-2 句把用户处境中的情绪张力具体化。
14. 微行动不是沉重建议。它可以是“写下一句最刺痛的比较”“暂停自我审判一分钟”“关掉触发源三天”“把抽象问题落到一个具体场景”“用一句话向老师/朋友请求反馈”等。

表达约束：
1. response_goal 不要写“先复述用户处境”“先表达听到”这类会导致下游机械复读的目标。
2. constraints 不要强制使用固定开头，不要要求或示例“我听到/听起来/确实很难受/确实让人/这确实”等模板句。
3. 如果选择 Restatement or Paraphrasing，不要要求逐字复述；应要求用 1-2 句基于用户原话展开其处境中的情绪张力。
4. 多轮对话中优先回应当前轮新信息，并明确是否要推进：安抚、肯定、建议、资源、安全确认或陪伴。
5. 在 comfort / affirm / normalize 的 constraints 中，不要把“建议”一概禁止到连微小稳定动作或温和探索入口都不能给。可以禁止沉重方案、说教和过早解决问题，但允许低负担支持动作。
6. 在 offer_next_step 的 constraints 中，除现实动作外，也要要求下游先承接用户的核心困惑、不甘或自我怀疑；不要只输出任务指令。
7. 对现实紧急/行政后果事件，不要在 prohibited_actions 或 constraints 中禁止“具体行动建议”“补考/缓考/教务/老师/辅导员”等现实帮助。可以限制为“只给一个下一步，避免吓人和过度承诺”。
8. constraints 只能写表达功能，不能写成品句、示例句或可被第三部分直接照抄的话术。"""


def strategy_user_prompt_v026(strategy_request: dict[str, Any]) -> str:
...
1. 情绪承接：用 1-2 句基于用户原话展开其处境中的心理负担、关系压力或自责机制；不要机械复读用户原句。
2. 轻度重构：帮助用户把自我攻击、嫉妒或挫败稍微换一个角度看，展开必须来自用户已经提供的信息。
3. 微行动：提供一个很小、今晚或此刻能做的动作，或一个温和探索入口。
4. 收束：降低自责，稳定预期。

避免过度推测，但允许 evidence-grounded elaboration：可以基于用户原话展开其处境中的心理负担、关系压力和自我责备机制；禁止的是添加未经证实的新事实、心理诊断、动机判断或责任归因。
constraints 不要写成品句，例如不要写“那道题没写出来，现在想起来真的很难受”。应写表达功能，例如“用 1-2 句基于用户原话展开其自责和后悔，不要照抄用户原句，不要给固定示例句”。
...
def contains_banned_practical_block(card: dict[str, Any]) -> bool:
    ...
    # Keep this deliberately narrow. Broad semantic regexes have caused false
    # positives such as treating "不要只做情绪认可而不提供现实帮助" as a ban on help.
    explicit_block_patterns = (
        r"(不能|禁止|不得|不要).{0,4}提供任何.{0,12}(补考|缓考|具体行动|行动建议|现实帮助|下一步)",
        r"(不能|禁止|不得|不要).{0,4}(提供|给出|提出).{0,8}(补考|缓考).{0,4}(建议|帮助|方案)",
        r"(不能|禁止|不得|不要).{0,4}(联系|询问).{0,8}(老师|教务|辅导员)",
        r"只能.{0,8}(情绪认可|情绪安抚|安慰)",
        r"先专注情绪认可$",
    )
    for item in items:
        ...
        if not text:
            continue
        if "不要只做" in text and "现实帮助" in text:
            continue
        if any(re.search(pattern, text) for pattern in explicit_block_patterns):
            return True
    return False
...
def validate_urgent_real_world_strategy(
    ...
    if not has_practical_strategy and not has_practical_intention:
        check.warn(
            "semantic warning: urgent real-world consequence cases should include a practical next-step strategy "
    ...
    if card.get("response_timing") == "respond_now" and card.get("support_intention") == "comfort":
        check.warn(
            "semantic warning: urgent real-world consequence cases may be under-helpful when framed as comfort-only/respond_now; "
            "use offer_next_step after brief emotional validation"
        )

    if contains_banned_practical_block(card):
        check.warn(
            "semantic warning: urgent real-world consequence cases may prohibit realistic next-step help; "
...
def api_strategy_decision_card(
    ...
    messages = [
        {"role": "system", "content": strategy_system_prompt_v026()},
        {"role": "user", "content": strategy_user_prompt_v026(strategy_request)},
    ]
