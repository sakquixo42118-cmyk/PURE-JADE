...
DEFAULT_DIMENSIONS = MODULE_DIR / "evaluation_dimensions.json"
DEFAULT_CASES = MODULE_DIR / "eval-test-cases-v0.1.json"
...
def recent_dialogue_window(record: dict[str, Any], turn_id: int, max_turns: int) -> list[dict[str, Any]]:
    """Return previous dialogue plus current user message, excluding target assistant output."""
    ...
    for item in dialogue_log:
        ...
        if not isinstance(item_turn, int) or not speaker or not content:
            continue
        if not (min_turn <= item_turn <= turn_id):
            continue
        if item_turn == turn_id and speaker != "user":
            continue
        window.append({"turn_id": item_turn, "speaker": speaker, "content": content})
    return window


def cases_from_conversation_record(
    record_path: Path,
    ...
    if not isinstance(record, dict):
        raise ValueError("conversation_record must be a JSON object")

    turn_records = [item for item in as_list(record.get("turn_records")) if isinstance(item, dict)]
    if requested_turn_id is not None:
        selected = [item for item in turn_records if item.get("turn_id") == requested_turn_id]
        if not selected:
            raise ValueError(f"turn_records does not contain turn_id={requested_turn_id}")
    else:
        selected = turn_records

    cases: list[dict[str, Any]] = []
    for turn_record in selected:
        turn_id = turn_record.get("turn_id")
        if not isinstance(turn_id, int):
            continue
        previous_record = find_turn_record(record, turn_id - 1) if turn_id > 1 else None
        case: dict[str, Any] = {
            "conversation_id": record.get("conversation_id") or turn_record.get("conversation_id"),
            "turn_id": turn_id,
            "schema_version": turn_record.get("schema_version") or record.get("schema_version", "0.2.1"),
            "dialogue": recent_dialogue_window(record, turn_id, max_recent_turns),
            "user_state_card": turn_record.get("user_state_card", {}),
            "strategy_decision_card": turn_record.get("strategy_decision_card", {}),
            "behavior_response_card": turn_record.get("behavior_response_card", {}),
            "_source_record_path": str(record_path),
        }
        if previous_record and isinstance(previous_record.get("user_state_card"), dict):
            case["previous_user_state_card"] = previous_record["user_state_card"]
        cases.append(case)
...
def load_api_config(args: argparse.Namespace) -> tuple[ApiConfig | None, list[str]]:
    ...
    url_value = args.api_url or os.environ.get("PURE_JADE_API_URL") or os.environ.get("LLM_BASE_URL") or DEFAULT_API_URL
    url = normalize_chat_completions_url(url_value)
    api_key = os.environ.get("PURE_JADE_API_KEY") or os.environ.get("LLM_API_KEY", "")
    model = args.api_model or os.environ.get("PURE_JADE_API_MODEL") or os.environ.get("LLM_MODEL") or DEFAULT_MODEL
    if args.api_temperature is not None:
        temperature = args.api_temperature
    elif os.environ.get("PURE_JADE_API_TEMPERATURE") is not None:
        temperature = read_float_env("PURE_JADE_API_TEMPERATURE", 0.0)
    else:
        temperature = read_float_env("LLM_TEMPERATURE", 0.0)
    if args.api_timeout is not None:
        timeout_seconds = args.api_timeout
    elif os.environ.get("PURE_JADE_API_TIMEOUT_SECONDS") is not None:
        timeout_seconds = read_int_env("PURE_JADE_API_TIMEOUT_SECONDS", 120)
    else:
        timeout_seconds = read_int_env("LLM_TIMEOUT_SECONDS", 120)
...
class CaseEvalResult:
    ...
    def map_to_v0_1_card(self, config: ApiConfig) -> dict[str, Any]:
        ...
        def to_1_5(score_1_10: float) -> int:
            return max(1, min(5, int((score_1_10 + 1) // 2)))
    ...
    def map_to_v0_2_card(self, config: ApiConfig) -> dict[str, Any]:
        ...
        # Only include dimensions that were actually evaluated in this run.
        score_basis: list[int] = []
        if any(dim in self.dimension_scores for dim in ("emotional_communication", "emotion_regulation")):
            score_basis.append(card["emotion_alignment"])
        if any(dim in self.dimension_scores for dim in ("coherence", "content_emotion_association")):
            score_basis.append(card["strategy_consistency"])
        if any(dim in self.dimension_scores for dim in ("content_emotion_association", "helpfulness")):
            score_basis.append(card["relevance"])
        if "adaptability" in self.dimension_scores:
            score_basis.append(card["naturalness"])
        if "constraint_compliance" in self.dimension_scores:
            score_basis.append(card["safety"])
        if state_update and state_update.score is not None:
            score_basis.append(card["state_update_validity"])
        if context_cont and context_cont.score is not None:
            score_basis.append(card["context_continuity"])
        if not score_basis:
            score_basis = [
        ...
        card["overall_score"] = max(1, min(5, round(sum(score_basis) / len(score_basis))))
        card["_score_basis"] = "evaluated_dimensions_only"
...
def main() -> None:
    ...
    for i, case in enumerate(cases):
        ...
        display_format = "v0.2" if args.format == "v0.2" else "v0.1"
        display_card = result.map_to_v0_2_card(api_config) if display_format == "v0.2" else v0_1_card

        print(
            f"[{i + 1}/{len(cases)}] {result.conversation_id} "
            f"overall_8d={result.overall_score} "
            f"overall_{display_format}={display_card['overall_score']} "
            f"({elapsed:.1f}s)"
        )
