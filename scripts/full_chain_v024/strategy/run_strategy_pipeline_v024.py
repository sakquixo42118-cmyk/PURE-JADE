...
def practical_constraints(strategy_request: dict[str, Any]) -> list[str]:
    ...
    constraints = [
        "先用一句话承接情绪，再给一个低负担现实下一步",
        "下游回复最多提出一个问题，且问题必须服务于下一步行动",
        "不承诺结果，不编造具体机构政策",
    ]
    if domain == "exam":
        constraints.append("现实下一步优先指向联系任课老师、教务或辅导员，询问补考/缓考/缺考处理流程")
    elif domain in {"deadline", "administrative"}:
        constraints.append("现实下一步优先指向查看官方截止要求或联系负责人员确认补救流程")
    else:
...
8. 如果 practical_context.real_world_consequence=true 且 practical_urgency 为 medium/high，不能只做情绪认可。应选择 advise 或 inform，primary/secondary 至少包含 Providing Suggestions 或 Information，response_timing 使用 offer_next_step；目标是“先稳住情绪，再给一个低负担现实下一步”。
9. 对考试/DDL/报名/预约等现实后果事件，正确边界是“不编造政策、不承诺结果、不责备用户”，不是“不能提供任何具体行动建议”。
...
如果 practical_context.real_world_consequence=true 且 practical_urgency 为 medium/high，请采用“简短情绪承接 + 一个现实下一步”的策略。不要把禁止项写成“不能提供补考/具体行动建议”；应禁止的是编造政策、保证结果或责备用户。
