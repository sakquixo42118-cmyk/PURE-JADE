...
def compress_repetitive_opening(text: str, behavior_request: dict[str, Any]) -> tuple[str, bool]:
    # v0.2.3 keeps natural API wording; local code no longer rewrites empathy openings into canned phrases.
    return text, False
    response = text.strip()
    if not response:
        return text, False
...
自然表达规则：
1. 不要用模板化开头。避免反复使用“我听到你说”“听起来”“我能感受到”“我理解你现在”“你提到”“确实很难受”等句式。
...
7. 如果 strategy_decision_card 的 response_timing 是 offer_next_step，或 primary/secondary_strategy 包含 Providing Suggestions / Information，text_response 必须包含一个具体、低负担、现实可执行的下一步；不能只做情绪认可或正常化。
"""
...
def validate_behavior_card(
    ...
    if strategy_requires_practical_step(strategy_card) and not response_has_practical_step(response):
        check.error(
            "strategy requires a concrete next step, but text_response only validates emotion; "
