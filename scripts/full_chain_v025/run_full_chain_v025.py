"""Run the full PURE-JADE v0.2.5 chain.

Chain:
    full_chain_v025/first -> full_chain_v025/strategy/run_strategy_pipeline_v025.py
    -> full_chain_v025/behavior/behavior_generator_api_schema_aligned_v025.py
    -> full_chain_v025/forth/run_full_evaluation.py
...
V025_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = Path(__file__).resolve().parents[2]
SCRIPTS_DIR = PROJECT_ROOT / "scripts"
FIRST_V025_DIR = V025_DIR / "first"
STRATEGY_V025_DIR = V025_DIR / "strategy"
BEHAVIOR_V025_DIR = V025_DIR / "behavior"
FORTH_V025_DIR = V025_DIR / "forth"

DEFAULT_ENV_FILE = PROJECT_ROOT / ".env"
DEFAULT_REFERENCES = PROJECT_ROOT / "examples" / "strategy-references-v0.1.json"
DEFAULT_DIMENSIONS = FORTH_V025_DIR / "evaluation_dimensions.json"
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "reports" / "full_chain_v025"
...
def default_conversation_id() -> str:
    return "chain_v025_" + time.strftime("%Y%m%d_%H%M%S")
...
def run_first_stage(args: argparse.Namespace, output_dir: Path) -> tuple[Path, dict[str, Any]]:
    first_main = load_module(
        "pure_jade_first_v025_main",
        FIRST_V025_DIR / "main.py",
        prepend_paths=[FIRST_V025_DIR],
    )
...
def prepare_working_record(source_record: Path, output_dir: Path) -> Path:
    working_record = output_dir / "conversation_record_v025_chain.json"
...
def run_strategy_stage(args: argparse.Namespace, working_record: Path, output_dir: Path, turn_id: int) -> dict[str, Any]:
    strategy_module = load_module(
        "pure_jade_strategy_v025",
        STRATEGY_V025_DIR / "run_strategy_pipeline_v025.py",
        prepend_paths=[STRATEGY_V025_DIR, V025_DIR],
    )
    api_client = load_module(
        "pure_jade_api_for_chain_v025",
        V025_DIR / "pure_jade_api.py",
        prepend_paths=[V025_DIR],
    )
...
def run_behavior_stage(args: argparse.Namespace, working_record: Path, output_dir: Path, turn_id: int) -> dict[str, Any]:
    behavior_module = load_module(
        "pure_jade_behavior_schema_aligned_v025",
        BEHAVIOR_V025_DIR / "behavior_generator_api_schema_aligned_v025.py",
        prepend_paths=[BEHAVIOR_V025_DIR, V025_DIR],
    )
...
def build_evaluation_cases(working_record: Path, output_dir: Path, turn_id: int) -> Path:
    ...
    write_json(
        ...
        {
            "schema_version": "0.2.1",
            "purpose": "full_chain_v025_current_turn_evaluation_input",
...
def build_all_turn_evaluation_cases(working_record: Path, output_dir: Path) -> Path:
    ...
    write_json(
        ...
        {
            "schema_version": "0.2.1",
            "purpose": "full_chain_v025_all_turns_evaluation_input",
            "source_record": str(working_record),
...
def run_evaluation_stage(
    ...
    command = [
        sys.executable,
        str(FORTH_V025_DIR / "run_full_evaluation.py"),
        "--cases",
        str(cases_path),
        "--dimensions",
        str(args.dimensions),
...
def write_conversation_summary_report(
    ...
    summary = {
        "report_type": "pure_jade_v025_conversation_summary",
        "schema_version": "0.2.1",
        "conversation_id": record.get("conversation_id"),
        "current_turn_id": record.get("current_turn_id"),
...
def write_dialogue_review_report(
    ...
    review = {
        "report_type": "pure_jade_v025_dialogue_review",
        "schema_version": "0.2.1",
...
def write_summary(
    ...
    summary = {
        "status": "pass",
        "chain": [
            "full_chain_v025/first",
            "full_chain_v025/strategy/run_strategy_pipeline_v025.py",
            "full_chain_v025/behavior/behavior_generator_api_schema_aligned_v025.py",
            "full_chain_v025/forth/run_full_evaluation.py",
        ],
        "modes": {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Pass-through dimensions file for full_chain_v025/forth/run_full_evaluation.py.",
    )
    parser.add_argument(
        "--eval-mode",
        choices=("fast", "full"),
