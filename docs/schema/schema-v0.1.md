...
<u>在当前实现和测试语境中，v0.1 按单轮 golden case 处理：每个 case 的可变输入主要是当前用户发言；系统提示词、枚举、Schema 和风险规则属于固定约束，不视为对话记忆。</u>

## 总体流程

```text
当前用户输入（v0.1 单轮 golden case 的主要变量）
-> 风险优先检测
-> 用户状态卡
-> 共情策略决策卡
-> 行为回应卡
...
<u>v0.1 中的 `problem_summary` 是对当前单轮输入的概括，不是 v0.2.1 中跨轮滚动维护的 `dialogue_summary`。</u>
